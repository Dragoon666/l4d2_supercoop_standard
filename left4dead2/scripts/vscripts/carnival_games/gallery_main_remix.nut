...
function SetTimer( timeLeft )
{
	...
	for ( local i = 0; i<3; i++ )
	{
		...
		{
			EntFire( CounterArray[i].GetName(), "SetPosition", fabs( ( (setval * 0.1) ) ) ) // subtracting from 1 to reverse
		}
...
function StartTimer( seconds )
{
	...
	EntFire( CounterArray[0].GetName(), "Startbackward", 0 )
}
...
function SetCounter( score )
{
	...
	for ( local i = 0; i<=3; i++ )
	{
		...
		EntFire( CounterArray[i+3].GetName(), "SetPositionImmediately", fabs( ( (setval * 0.1) ) ) ) // subtracting from 1 to reverse
	}
}
