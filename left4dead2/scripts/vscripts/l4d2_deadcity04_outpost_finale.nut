...
function AbortDelay(){}  	// only defined during a delay, in l4d2_deadcity04_delay.nut
function EndDelay(){}		// only defined during a delay, in l4d2_deadcity04_delay.nut
...
function GasCanPoured()
{
	...
	if ( GasCansPoured == 3)
	{
		EntFire( "soldier_fuel_setup1", "playsound" )
	}
	
	if ( GasCansPoured == 6)
	{
		EntFire( "soldier_fuel_setup2", "playsound" )
	}
	
	if ( GasCansPoured == 9)
	{
		EntFire( "soldier_fuel_setup3", "playsound" )
	}

    if ( GasCansPoured == 10)
	{
		EntFire( "soldier_fuel_setup4", "playsound" )
	}

    if ( GasCansPoured == 12)
	{
		EntFire( "soldier_fuel_setup5", "playsound" )
	}

    if ( GasCansPoured == NumCansNeeded )
    {
        Msg(" needed: " + NumCansNeeded + "\n") 
        EntFire( "relay_endgame", "trigger" )
    }
...
{
	printl( "Beginning outpost finale stage " + num + " of type " + type );
