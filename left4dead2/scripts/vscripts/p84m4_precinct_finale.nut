...
Msg("-------- PRECINCT 84 FINALE SCRIPT INTIATED --------------\n")
//-----------------------------------------------------
PANIC <- 0
TANK <- 1
DELAY <- 2
...
function AbortDelay(){}  	// only defined during a delay, in p84m4_delay.nut
function EndDelay(){}		// only defined during a delay, in p84m4_delay.nut
...
function GasCanPoured()
{
	...
	// Add to a math_counter which sends an outValue to a logic_case
	// Which plays sound from the "Police officer" how full the car is.
	if ( Director.IsSinglePlayerGame() )
{
    Msg("-- Added to SP counter--\n")
	EntFire("counterEscapeVehicleFueled_sp","add","1")
	
}  else {

    Msg("-- Added to MP counter--\n")
    EntFire("counterEscapeVehicleFueled_mp","add","1")
