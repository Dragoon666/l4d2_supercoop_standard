...
{	
	...
	//This delay ensures that the train has completed its rotation - If the train reaches its location during the tanks portion, a dynamic in the map will allow this stage to be skipped.
	...
	A_CustomFinaleValue5 = 9999 //Fire off the canister panic via input to finale: AdvanceFinaleState
	...
	//The previous stage should be physically skipped via the finale input AdvanceFinaleState - important to fire FinaleEscapeVehicleReadyForSurvivors **AFTER** AdvanceFinaleState
...
//Checks to see if the right amount of stages in the event have passed to begin the canister event
//if ( ReadyToGo == HowManyStages )
    //{
    //    Msg("Canister Dynamic Beginning\n") 
    //    EntFire( "canister_event_begin", "trigger", "", 1 )
    //}
...
if ( ReadyToGo == TankPostStage )
    {
		...
		EntFire( "rot_train", "SetSpeed", "3", 1 )
    }
